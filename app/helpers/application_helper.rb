...
module ApplicationHelper
  ...
  def logo_path(object, options = {})
    size = options[:size] || 180
    default_image = options[:default] || 'mp'
    base_url = 'https://secure.gravatar.com/avatar'
    base_url_params = "?s=#{size}&d=#{default_image}"

    if object.respond_to?(:logo) && object.logo.attached? && object.logo.variable?
      object.logo.variant(resize_to_fill: [size, size, { gravity: 'Center' }])
    else
      "#{base_url}/00000000000000000000000000000000#{base_url_params}"
    end
  end
